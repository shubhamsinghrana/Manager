...
public class ThursdayTimeTable extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_thursday_time_table);
...
//daySpinner=findViewById(R.id.daySpinner);
      ...
      //  mondaySaveBtn.setVisibility(View.INVISIBLE);
        //  daySpinner=findViewById(R.id.daySpinner);
        if(fetchvalues("Thursday"))
        {
                   ...
                   // mondaySaveBtn.setVisibility(View.VISIBLE);
                    ...
                    try {
                        ...
                        try {
                            ...
                            try {
                                Toast.makeText(ThursdayTimeTable.this, period1, Toast.LENGTH_SHORT).show();
                                mydb.execSQL("insert into timetable1(day,period1,period2,period3,period4,period5) values(?,?,?,?,?,?)",
                                        new Object[]{"Thursday", period1, period2, period3, period4, period5});
                            } catch (Exception e) {
                                Toast.makeText(ThursdayTimeTable.this, "Unable to insert in table" + e.getMessage(), Toast.LENGTH_LONG).show();
                            }
                            mydb.close();
                            Toast.makeText(ThursdayTimeTable.this, "Saved Sucssefully!!", Toast.LENGTH_SHORT).show();
                            ThursdayTimeTable.this.finish();


                        } catch (Exception e) {
                            Toast.makeText(ThursdayTimeTable.this, "Unable to create table!!", Toast.LENGTH_SHORT).show();
                        }

                    } catch (Exception e) {
                        Toast.makeText(ThursdayTimeTable.this, "Unable to Create Database!!", Toast.LENGTH_SHORT).show();
                    }
                }
                else
                {
                    Toast.makeText(ThursdayTimeTable.this, "Please Fill All Details", Toast.LENGTH_SHORT).show();
                ...
                if (a == 1 && b == 1 && c == 1 && d == 1 && e == 1)
                {
                    ...
                    try {
                        mydb.execSQL("update timetable1 set period1=?, period2=?, period3=?,period4=?, period5=?" +
                                "where day=?",new String[]{period1, period2, period3, period4, period5,"Thursday"});


                        mydb.close();
                        Toast.makeText(ThursdayTimeTable.this, "Updated Successfully", Toast.LENGTH_SHORT).show();

                    }
                    catch(Exception e)
                    {
                        Toast.makeText(ThursdayTimeTable.this, "Error in Creating Table due to " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                }
                else
                {

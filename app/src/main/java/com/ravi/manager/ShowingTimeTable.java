...
public class ShowingTimeTable extends AppCompatActivity {
...
int a=0,b=0,c=0,d=0,e=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_showing_time_table);
...
//daySpinner=findViewById(R.id.daySpinner);
     ...
     //   spinner1.setSelection(3);
        ...
        updateSaveBtn.setVisibility(View.GONE);
        if(fetchvalues("Monday"))
        {
                        ...
                        try {
                            ...
                            try {
                                ...
                                mydb.execSQL("insert into timetable1(day,period1,period2,period3,period4,period5) values(?,?,?,?,?,?)",
                                        new Object[]{"Monday", period1, period2, period3, period4, period5});
                            } catch (Exception e) {
                                Toast.makeText(ShowingTimeTable.this, "Unable to insert in table" + e.getMessage(), Toast.LENGTH_LONG).show();
                            }
                            mydb.close();
                            Toast.makeText(ShowingTimeTable.this, "Saved Sucssefully!!", Toast.LENGTH_SHORT).show();
                            ShowingTimeTable.this.finish();


                        } catch (Exception e) {
                            Toast.makeText(ShowingTimeTable.this, "Unable to create table!!", Toast.LENGTH_SHORT).show();
                        }

                    } catch (Exception e) {
                        Toast.makeText(ShowingTimeTable.this, "Unable to Create Database!!", Toast.LENGTH_SHORT).show();
                    }}
                    else
                {

                    Toast.makeText(ShowingTimeTable.this, "Please Fill All Details", Toast.LENGTH_SHORT).show();
                }
                }
        ...
        if (a == 1 && b == 1 && c == 1 && d == 1 && e == 1)
        {
            ...
            try {
                mydb.execSQL("update timetable1 set period1=?, period2=?, period3=?,period4=?, period5=?" +
                        "where day=?",new String[]{period1, period2, period3, period4, period5,"Monday"});


                mydb.close();
                Toast.makeText(ShowingTimeTable.this, "Updated Successfully", Toast.LENGTH_SHORT).show();

            }
            catch(Exception e)
            {
                Toast.makeText(ShowingTimeTable.this, "Error in Creating Table due to " + e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        }
        else
        {
